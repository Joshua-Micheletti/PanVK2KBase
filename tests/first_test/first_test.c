...
/* Real vendored headers, r44p0 / UK 44.10 - see
 * third_party/kbase-uapi-r44p0/README.md for provenance. mali_kbase_ioctl.h
 * internally pulls in csf/mali_kbase_csf_ioctl.h and jm/mali_kbase_jm_ioctl.h
 * itself, so one include is enough. */
...
int main(void) {
  ...
  struct kbase_ioctl_version_check ver = {
      .major = 0, /* VERIFY: some kbase versions want you to send
  ...
  if (ret < 0) {
    fprintf(stderr, "VERSION_CHECK failed: %s (errno %d)\n", strerror(errno),
            errno);
    fprintf(stderr, "A failure here almost always means either the ioctl "
  ...
  if (props_size == 0) {
    fprintf(stderr, "couldn't determine gpuprops size from either "
  ...
  //parse_mali_gpu_props(props_buf, props_size);
